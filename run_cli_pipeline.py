#!/usr/bin/env python3
"""Run the local CLI bridge and expose it via ngrok."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the local Codex/Gemini CLI bridge and expose it via one ngrok URL."
    )
    parser.add_argument(
        "--providers",
        default=os.getenv("CLI_BRIDGE_PROVIDERS", "codex,gemini"),
        help="Comma-separated providers to enable (default: codex,gemini)",
    )
    parser.add_argument(
        "--bridge-port",
        type=int,
        default=int(os.getenv("CLI_BRIDGE_PORT", "8350")),
        help="Port for the local CLI bridge (default: 8350)",
    )
    parser.add_argument(
        "--startup-timeout",
        type=int,
        default=int(os.getenv("CLI_BRIDGE_STARTUP_TIMEOUT", "120")),
        help="Seconds to wait for CLI bridge startup (default: 120)",
    )
    parser.add_argument(
        "--region",
...
def wait_for_health(url: str, timeout_sec: int, proc: subprocess.Popen) -> None:
    ...
    while time.time() < deadline:
        if proc.poll() is not None:
            raise RuntimeError("CLI bridge exited before becoming healthy.")
    ...
    raise RuntimeError(
        f"Timed out waiting for CLI bridge health at {url}. Last error: {last_error}"
    )
...
def main() -> int:
    ...
    bridge_env["CLI_BRIDGE_PROVIDERS"] = args.providers
    ...
    try:
        ...
        terminate_process(bridge_proc, "cli bridge")
        return 1

    print("CLI bridge is healthy. Starting ngrok tunnel...")
    ...
    while not stop:
        ...
        if bridge_rc is not None:
            print("CLI bridge stopped unexpectedly.", file=sys.stderr)
